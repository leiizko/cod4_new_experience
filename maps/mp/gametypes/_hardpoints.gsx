...
losRadiusDamage(pos, radius, max, min, owner, eInflictor)
{
	...
	for (i = 0; i < ents.size; i++)
	{
		...
		dist = distanceSquared(pos, ents[i].damageCenter);
...
getBestPlaneDirection( hitpos )
{
	...
	for ( i = 0; i < 360; i += 10 )
	{
		...
		if ( trace["fraction"] > bestanglefrac )
		{
			...
			if ( trace["fraction"] >= 1 )
				fullTraceResults[ fullTraceResults.size ] = i;
		}
...
giveHardpointItemForStreak()
{
	...
	{
		case 3:
			self giveHardpoint( "radar_mp", self.cur_kill_streak );
			break;
		
		case 5:
			self giveHardpoint( "airstrike_mp", self.cur_kill_streak );
			break;
			
		case 9:
			self giveHardpoint( "helicopter_mp", self.cur_kill_streak );
			break;
		
		default:
			if( self.cur_kill_streak % 5 == 0 )
				self streakNotify( self.cur_kill_streak );
	}
}
