...
init()
{
	...
	thread code\init::GloballogicInit();
...
spawnPlayer()
{
	...
	if ( level.inPrematchPeriod )
	{
		...
		thread maps\mp\gametypes\_hud_message::oldNotifyMessage( level.dvar[ "intro_text" ], level.dvar[ "website" ], undefined, undefined, undefined, level.dvar[ "intro_time" ] );
	...
	else
	{
		...
		self enableWeapons();
		if ( !self.hasSpawned && game["state"] == "playing" )
		{
			thread maps\mp\gametypes\_hud_message::oldNotifyMessage( level.dvar[ "intro_text" ], level.dvar[ "website" ], undefined, undefined, undefined, level.dvar[ "intro_time" ] );
...
spectatorThirdPersonness()
{
	...
	while(1)
	{
		if ( self getSpectatorClient() != player )
			player = self getSpectatorClient();
...
Callback_PlayerDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	...
	// Damage event
	self thread code\events::onPlayerDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime );
...
//			}
		...
		if ( !level.dvar[ "disable_jugger" ] && isdefined(eAttacker) && eAttacker != self )
		{
			hasBodyArmor = false;
			...
			else
			{
				...
				else
				{
					...
					if( sMeansOfDeath == "MOD_MELEE" )
						attacker thread maps\mp\gametypes\_rank::giveRankXP( "kill", value, true );
					else
