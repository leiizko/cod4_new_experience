...
watchGrenadeUsage()
{
	...
	if( !level.dvar[ "disable_claymore" ] && !level.dvar[ "disable_c4" ] )
...
detectIconWaiter( detectTeam )
{
	...
	while( !level.gameEnded )
	{
		...
		if( !isDefined( player ) || !isDefined( player.detectExplosives ) || !isDefined( player.team ) )
			continue;
